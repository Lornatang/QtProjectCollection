...
#include "../include/windoweffect.h"
...
void FramelessWindow::changeEvent(QEvent *event) {
  if (event->type() == QEvent::WindowStateChange) {
    RefreshStyle(titleBar->findChild<QToolButton *>("maxToolButton"),
                 "MaxState", isMaximized());
  }
...
void FramelessWindow::monitorNCCALCSIZE(MSG *msg) {
  ...
  // qDebug() << params.rgrc[0].left << params.rgrc[0].top <<
